...
At very first, the data is normalized in the model using a Keras lambda layer (code line 47). 
Then I use a cropping layer(code line 49) to crop the images in Keras. 

Following them are five convolutional layers with 5x5 or 3x3 filter sizes and RELU activation function(code line 50 - 54).

After the five convolutional layers, there is a flatten layer to flatten parameter(code line 55)

And then , there are four fully connected layers with only one output at last(code line 56, 57, 60, 61)

#### 2. Attempts to reduce overfitting in the model

The model contains dropout layers in order to reduce overfitting (code lines 59). 
